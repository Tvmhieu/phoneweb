...
export default function AdminDashboard() {
  ...
  // =============== CÁC HìM XỬ LÝ ACTION ===============
  const [uploading, setUploading] = useState(false);

  const formatCustomer = (order: any) => {
    if (!order.user || order.user.isDeleted) {
      return { 
        name: order.user?.name ? `${order.user.name} (Đã xóa)` : "Người dùng �ã xóa", 
    ...
    return { name, contact: contact || "Chưa có thông tin liên h�!" };
  };
  ...
  const saveProduct = async (e: React.FormEvent) => {
      ...
      } catch(e) { alert("L�i kết n�i"); }
  };

  const deleteProduct = async (id: number) => {
    if (!confirm("Bạn có chắc chắn mu�n xóa sản phẩm này?")) return;
    try {
  ...
  const deleteUser = async (id: number) => {
    if (!confirm("Bạn có chắc chắn mu�n xóa nhân sự này?")) return;
    try {
  ...
  const deleteSaleOrder = async (id: number) => {
    if (!confirm("Bạn có chắc chắn mu�n xóa hóa �ơn bán hàng này?")) return;
    try {
  ...
  const deleteWarranty = async (id: number) => {
    if (!confirm("Bạn có chắc chắn mu�n X�A VĨNH VI�N sự c� này khỏi h�! th�ng?")) return;
    try {
      const res = await fetch(`/api/admin/warranties?id=${id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
        alert("Đã xóa vĩnh vi�&n!");
        loadWarranties();
      } else alert(data.message);
    } catch(e) {}
  };
  ...
  const deleteContact = async (id: number) => {
    if (!confirm("Bạn có chắc chắn mu�n xóa yêu cầu liên h�! này?")) return;
    try {
  ...
  if (!hydrated || !userRole) return <div className="p-5 text-center text-muted"><i className="bi bi-shield-lock d-block fs-1"></i>Đang kiểm tra quyền truy cập...</div>;
              ...
              <ul className="nav flex-column mb-2">
                <li className="nav-item">
                  <button className={`nav-link w-100 text-start border-0 bg-transparent py-2 px-4 d-flex align-items-center transition-premium ${activeTab === "WARRANTIES" ? "active-premium-light" : "text-secondary"}`} onClick={() => setActiveTab("WARRANTIES")}>
                    ...
                    <span className="small fw-bold">Bảo hành & Hỗ trợ</span>
                  </button>
                </li>
                <li className="nav-item">
            ...
            <div className="sidebar-footer p-3 border-top border-light flex-shrink-0 bg-white shadow-lg-top">
               ...
               <div className="d-flex gap-2">
                 ...
                 <button onClick={logout} className="btn btn-danger flex-fill py-1 rounded-3 shadow-sm small fw-bold" style={{ fontSize: '0.8rem' }}>
                   <i className="bi bi-power me-1"></i> Đăng xuất
                 </button>
               </div>
            </div>
          </div>
        ...
        <div className="col-lg-10 offset-lg-2 pt-0 pt-lg-4 px-0 px-md-4 d-flex flex-column min-vh-100">
             ...
             <div className="animate__animated animate__fadeIn">
                 <div className="d-flex justify-content-between align-items-center mb-5">
                    ...
                    <button className="btn btn-white shadow-sm border py-2 px-4 rounded-3 fw-bold" onClick={loadStats}>
                        <i className="bi bi-arrow-clockwise me-2 text-primary"></i>Làm mới dữ liệu
                    </button>
                 </div>
                                 ...
                                 <div className="row g-3">
                                     ...
                                     <div className="col-md-6">
                                      ...
                                      <div className="col-md-3">
                                          <label className="form-label small fw-bold">Thương hi�!u</label>
                                      ...
                                      <div className="col-md-3">
                                          ...
                                          <select className="form-select" value={editingProduct.category || "SERVER"} onChange={e => setEditingProduct({...editingProduct, category: e.target.value})}>
                                              <option value="IPHONE">i�n tho�i iPhone</option>
                                      ...
                                      <div className="col-md-3">
                                          <label className="form-label small fw-bold text-info">HiỒn th�9 Storefront</label>
                                          <select className="form-select border-info fw-bold" value={editingProduct.isVisible !== false ? "SHOW" : "HIDE"} onChange={e => setEditingProduct({...editingProduct, isVisible: e.target.value === "SHOW"})}>
                                              <option value="SHOW">HiỒn th�9</option>
                                      ...
                                      <div className="col-md-2">
                                          <label className="form-label small fw-bold">T�n kho</label>
                                     ...
                                     <div className="col-12 mt-3">
                                         <label className="form-label small fw-bold"><i className="bi bi-file-text me-1"></i>Mô tả sản phẩm & Gi�:i thi�!u chức nĒng</label>
                                         <textarea className="form-control" rows={6} value={editingProduct.description || ""} onChange={e => setEditingProduct({...editingProduct, description: e.target.value})} placeholder="Nhập mô tả chi tiết, gi�:i thi�!u chức nĒng, thông s� kỹ thuật (xu�ng dòng �Ồ trình bày �ẹp)..."></textarea>
                                         <div className="form-text">Mô tả này sẽ hiỒn th�9 trực tiếp tại trang chi tiết sản phẩm.</div>
                                     </div>
                                     <div className="col-12 mt-4 text-end">
                                         <button type="button" className="btn btn-light border me-2 px-4 shadow-sm" onClick={() => setEditingProduct(null)}>Hủy bỏ</button>
                                         <button type="submit" className="btn btn-primary fw-bold px-5 shadow-sm">LƯU CƠ S�~ DỮ LI� U</button>
                                     </div>
                                 </div>
                             </form>
                         </div>
                      ...
                      <div className="table-responsive">
                          <table className="table table-hover align-middle mb-0">
                              <thead className="bg-light">
                                  <tr>
                                      <SortHeader label="THIẾT B�`" sortKey="name" currentSort={productSort} onSort={handleSortProduct} className="px-4" />
                              ...
                              <tbody>
                                 ...
                                 {!loading && products.length === 0 && (
                                   <tr>
                                    <td colSpan={6} className="text-center py-5 text-muted">Kho hàng �ang tr�ng hoặc chưa tải �ược dữ li�!u.</td>
                                   </tr>
                                 )}
                                 {getFilteredProducts(products).map(p => (
                                     <tr key={p.id}>
                                         ...
                                         <td className="text-danger fw-bold">{p.price?.toLocaleString()}�</td>
                                         ...
                                         <td className="text-end px-4">
                                             ...
                                             >
                                               <option value="SHOW">HiỒn th�9</option>
                                               <option value="HIDE">Chế ��" Ẩn</option>
                                             </select>
            ...
            <div>
                <div className="d-flex justify-content-between align-items-center mb-4">
                  ...
                  <div className="d-flex gap-3">
                    <div className="input-group" style={{ width: "300px" }}>
                       <span className="input-group-text bg-white border-end-0"><i className="bi bi-search"></i></span>
                       <input type="text" className="form-control border-start-0" placeholder="Tìm mã �ơn, tên khách..." value={saleSearch} onChange={e => setSaleSearch(e.target.value)} />
                    </div>
                    <div className="alert alert-info py-2 px-3 mb-0 small d-none d-md-block">
                      <i className="bi bi-info-circle me-1"></i> Doanh thu ch�0 �ược c�"ng sau khi �ơn hàng <b>Đã thanh toán</b>.
                    </div>
                  </div>
                </div>

                {/* H�A ĐƠN CHƯA THANH TOÁN */}
                <div className="card shadow-sm border-0 rounded-3 mb-5 overflow-hidden">
                    <div className="card-header bg-warning text-dark fw-bold py-3 d-flex justify-content-between">
                      ...
                      <small className="opacity-75">Click vào tiêu �ề �Ồ sắp xếp</small>
                    </div>
                    <div className="table-responsive">
                        <table className="table hover align-middle mb-0">
                            <thead className="bg-light">
                                <tr>
                                    <SortHeader label="MÒ ĐƠN" sortKey="id" currentSort={saleSort} onSort={handleSortSale} className="px-4" />
                                    <SortHeader label="KHÁCH HìNG" sortKey="user" currentSort={saleSort} onSort={handleSortSale} />
                                    <th className="text-dark fw-bold">Đ�`A CH�� GIAO HìNG</th>
                                    <SortHeader label="T�NG TIỬN" sortKey="total" currentSort={saleSort} onSort={handleSortSale} />
                                    <th className="text-center">BẢO HìNH</th>
                                    <SortHeader label="NGìY MUA" sortKey="createdAt" currentSort={saleSort} onSort={handleSortSale} />
                                    <th className="text-end px-4 py-3 text-dark fw-bold">THAO TÁC</th>
                                </tr>
                            </thead>
                             <tbody>
                                   ...
                                   return (
                                     <tr key={s.id}>
                                         ...
                                         <td>
                                          ...
                                          <td className="text-danger fw-bold">{s.total.toLocaleString()} �</td>
                                         ...
                                         <td className="text-end px-4">
                                             ...
                                             <button className="btn btn-outline-danger btn-sm me-2" onClick={() => updateSaleStatus(s.id, "CANCELLED")}>Hủy �ơn</button>
                                             <button className="btn btn-sm btn-outline-danger" title="Xóa vĩnh vi�&n" onClick={() => deleteSaleOrder(s.id)}><i className="bi bi-trash"></i></button>
                                         </td>
                                     </tr>
                                   );
                                 })}
                                 {sales.filter(s => s.status === "PENDING").length === 0 && <tr><td colSpan={5} className="text-center p-4 text-muted">Không có �ơn hàng nào �ang chờ.</td></tr>}
                             </tbody>
                        </table>
                    </div>
                </div>

                {/* H�A ĐƠN CH�S VẬN CHUY�N */}
                <div className="card shadow-sm border-0 rounded-3 mb-5 overflow-hidden">
                    <div className="card-header bg-info text-white fw-bold py-3 d-flex justify-content-between">
                      <span><i className="bi bi-truck me-2"></i>Hóa Đơn Chờ Vận ChuyỒn (Paid)</span>
                    </div>
                    <div className="table-responsive">
                        <table className="table hover align-middle mb-0">
                            <thead className="bg-light">
                            ...
                            <tbody>
                                {getFilteredSales(sales.filter(s => s.status === "PAID")).map(s => {
                                  ...
                                  return (
                                    <tr key={s.id}>
                                        ...
                                        <td className="text-primary fw-bold">{s.total.toLocaleString()} �</td>
                                        <td className="text-center">
                                           {s.items?.map(it => (
                                               <div key={it.id} className="small text-muted">{it.product?.warrantyMonths || 12}T</div>
                                           ))}
                                ...
                                {sales.filter(s => s.status === "PAID").length === 0 && <tr><td colSpan={6} className="text-center p-4 text-muted">Không có �ơn hàng nào �ang chờ vận chuyỒn.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>

                {/* H�A ĐƠN ĐÒ HOìN THìNH */}
                <div className="card shadow-sm border-0 rounded-3 overflow-hidden">
                    <div className="card-header bg-success text-white fw-bold py-3"><i className="bi bi-cash-stack me-2"></i>L�9ch Sử Hóa Đơn Đã Hoàn Thành (Delivered)</div>
                    <div className="table-responsive">
                        <table className="table hover align-middle mb-0">
                            <thead className="bg-light">
                                <tr>
                                    <SortHeader label="MÒ ĐƠN" sortKey="id" currentSort={saleSort} onSort={handleSortSale} className="px-4" />
                                    <SortHeader label="KHÁCH HìNG" sortKey="user" currentSort={saleSort} onSort={handleSortSale} />
                                    <th className="text-dark fw-bold text-center">Đ�`A CH�� GIAO</th>
                                    <SortHeader label="T�NG TIỬN" sortKey="total" currentSort={saleSort} onSort={handleSortSale} />
                                    <th className="text-center">BẢO HìNH</th>
                                    <SortHeader label="NGìY MUA" sortKey="createdAt" currentSort={saleSort} onSort={handleSortSale} />
                            ...
                            <tbody>
                                ...
                                {sales.filter(s => s.status === "DELIVERED").length === 0 && <tr><td colSpan={6} className="text-center p-4 text-muted">Chưa có hóa �ơn nào hoàn thành.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>
          ...
          {activeTab === "USERS" && (
            <div className="animate__animated animate__fadeIn">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                      <h3 className="fw-bold mb-1 text-dark">Quản Lý Đ�"i Ngũ Nhân Sự</h3>
                      <p className="text-muted small mb-0">Điều hành và phân quyền truy cập h�! th�ng ABC XYZ</p>
                    </div>
                    <div className="d-flex gap-3">
                ...
                <div className="row g-4 mb-5">
                   <div className="col-md-4">
                      <div className="p-4 bg-white rounded-4 shadow-sm border-0 border-start border-danger border-5 h-100">
                         <div className="text-secondary small fw-bold text-uppercase mb-2">Quản tr�9 viên (Admin)</div>
                         <div className="d-flex align-items-center">
                ...
                {editingUser && (
                  <div className="card shadow-lg border-0 mb-5 p-4 border-top border-4 border-danger bg-white animate__animated animate__slideInDown">
                      <div className="d-flex justify-content-between align-items-center mb-4">
                        <h5 className="fw-bold text-danger mb-0">{editingUser.id ? `Ch�0nh sửa h� sơ: ${editingUser.name}` : "Thiết lập tài khoản Staff m�:i"}</h5>
                      ...
                      <form onSubmit={saveUser}>
                          <div className="row g-4">
                              ...
                              <div className="col-md-4">
                                <label className="form-label small fw-bold">Email (ĐĒng nhập)</label>
                              ...
                              <div className="col-md-4">
                                <label className="form-label small fw-bold">Mật khẩu {editingUser.id && "(Bỏ tr�ng nếu không ��"i)"}</label>
                              ...
                              <div className="col-md-4">
                                ...
                                <select className="form-select py-2 shadow-sm" style={{ borderLeft: '5px solid #dc3545' }} value={editingUser.role} onChange={e => setEditingUser({...editingUser, role: e.target.value})}>
                                    <option value="ADMIN">ADMIN (Quản tr�9 h�! th�ng)</option>
                              ...
                              <div className="col-md-4 d-flex align-items-end">
                                <div className="d-flex gap-2 w-100">
                                  ...
                                  <button type="submit" className="btn btn-danger flex-fill py-2 fw-bold shadow-sm">LƯU H� SƠ <i className="bi bi-check2-circle ms-1"></i></button>
                                </div>
                              </div>
                          </div>
                      </form>
                  ...
                  <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                      <thead className="bg-light bg-opacity-50">
                        <tr>
                          <SortHeader label="NH�N VI�`N" sortKey="name" currentSort={staffSort} onSort={handleSortStaff} className="px-4 py-3" />
                          <SortHeader label="TH�NG TIN LI�`N H� " sortKey="email" currentSort={staffSort} onSort={handleSortStaff} />
                          <SortHeader label="PH�NG BAN" sortKey="companyName" currentSort={staffSort} onSort={handleSortStaff} />
                          <SortHeader label="PH�N QUYỬN" sortKey="role" currentSort={staffSort} onSort={handleSortStaff} />
                          <th className="text-end px-4 py-3 text-dark fw-bold">QUẢN TR�`</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          return (
                            <tr key={u.id} className="transition-premium">
                              ...
                              <td>
                                ...
                                <div className="x-small text-muted">H�! th�ng ABC XYZ Internal</div>
                              </td>
                              <td>
                              ...
                              <td className="text-end px-4">
                                <div className="btn-group shadow-sm rounded-3 overflow-hidden">
                                  <button className="btn btn-sm btn-white border-end" title="Ch�0nh sửa" onClick={() => setEditingUser(u)}><i className="bi bi-pencil-square text-primary"></i></button>
            ...
            <div>
                ...
                {editingUser && (
                  <div className="card shadow-sm border-0 mb-5 p-4 border-start border-5 border-dark bg-white">
                      <h5 className="fw-bold text-dark mb-4">{editingUser.id ? "Sửa thông tin khách hàng" : "Tạo tài khoản khách hàng m�:i"}</h5>
                      <form onSubmit={saveUser}>
                          <div className="row g-3">
                              <div className="col-md-4">
                              ...
                              <div className="col-md-4">
                                <label className="form-label small fw-bold">Email (Dùng �Ēng nhập)</label>
                              ...
                              <div className="col-md-4">
                                <label className="form-label small fw-bold">Mật khẩu {editingUser.id && "(Tr�ng = giữ nguyên)"}</label>
                              ...
                              <div className="col-md-4">
                                <label className="form-label small fw-bold">S� �i�!n thoại</label>
                              ...
                              <div className="col-md-8">
                                <label className="form-label small fw-bold">Đ�9a ch�0 giao hàng / VĒn phòng</label>
                                <input type="text" className="form-control" placeholder="VD: 123 Nguy�&n VĒn Linh, Q7, TP.HCM" value={editingUser.address || ""} onChange={e => setEditingUser({...editingUser, address: e.target.value})}/>
                              </div>
                              ...
                              <div className="col-md-4 d-flex align-items-end">
                                  ...
                                  <button type="submit" className="btn btn-dark px-4 fw-bold">LƯU KHÁCH HìNG</button>
                              </div>
                          </div>
                      </form>
                  </div>
                  ...
                  <div className="table-responsive">
                    <table className="table hover align-middle mb-0">
                      <thead className="bg-light text-uppercase">
                        <tr>
                          <SortHeader label="KHÁCH HìNG" sortKey="name" currentSort={customerSort} onSort={handleSortCustomer} className="px-4" />
                          <SortHeader label="EMAIL" sortKey="email" currentSort={customerSort} onSort={handleSortCustomer} />
                          <SortHeader label="C�NG TY" sortKey="companyName" currentSort={customerSort} onSort={handleSortCustomer} />
                          <th className="py-3 px-2 text-dark fw-bold">PHONE</th>
                          <th className="py-3 px-2 text-dark fw-bold">Đ�`A CH��</th>
                          <SortHeader label="NGìY TẠO" sortKey="createdAt" currentSort={customerSort} onSort={handleSortCustomer} />
                          <th className="text-end px-4 py-3 text-dark fw-bold">THAO TÁC</th>
                        </tr>
                      </thead>
                      <tbody>
          ...
          {activeTab === "WARRANTIES" && (
              <div>
                  ...
                  {/* Stage 1: Tiếp nhận */}
                  <div className="card shadow-sm border-0 rounded-3 mb-5 overflow-hidden">
                      <div className="card-header bg-warning text-dark fw-bold py-3"><i className="bi bi-mailbox2 me-2"></i>1. Tiếp nhận & Chờ kiỒm tra</div>
                      <div className="table-responsive">
                          <table className="table hover align-middle mb-0">
                              <thead className="bg-light">
                                  <tr>
                                      <SortHeader label="TICKET" sortKey="id" currentSort={warrantySort} onSort={handleSortWarranty} className="px-4" />
                                      <SortHeader label="TH�NG TIN" sortKey="user" currentSort={warrantySort} onSort={handleSortWarranty} />
                                      <SortHeader label="THIẾT B�`" sortKey="product" currentSort={warrantySort} onSort={handleSortWarranty} />
                                      <th className="px-4 py-3 text-dark fw-bold">M� TẢ L�I</th>
                                      <th className="text-end px-4 py-3 text-dark fw-bold">THAO TÁC</th>
                                  </tr>
                              </thead>
                              <tbody>
                                  {getFilteredWarranties(warranties.filter(w => w.status === "PENDING")).map(w => (
                                      <tr key={w.id}>
                                          ...
                                          <td className="text-end px-4">
                                              <button className="btn btn-sm btn-outline-info fw-bold" onClick={() => updateWarrantyStatus(w.id, 'CHECKING')}>Bắt �ầu kiỒm tra <i className="bi bi-arrow-right"></i></button>
                                          </td>
                                      </tr>
                                  ))}
                                  {warranties.filter(w => w.status === "PENDING").length === 0 && <tr><td colSpan={5} className="text-center p-4 text-muted small">Mọi máy �ã �ược tiếp nhận xong.</td></tr>}
                              </tbody>
                          </table>
                      </div>
                  </div>
                          ...
                          <table className="table hover align-middle mb-0">
                              <thead className="bg-light">
                                  <tr>
                                      ...
                                      <th>THIẾT B�` / KHÁCH</th>
                                      <th>HƯ�aNG GIẢI QUYẾT & GHI CH�a</th>
                                      <th className="text-center">T�RNH TRẠNG</th>
                              ...
                              <tbody>
                                  {getFilteredWarranties(warranties.filter(w => w.status === "CHECKING" || w.status === "PROCESSING")).map(w => (
                                      <tr key={w.id}>
                                          ...
                                          <td>
                                            <input type="text" className="form-control form-control-sm" placeholder="Ghi chú hư�:ng sửa chữa..." defaultValue={w.resolution || ""} onBlur={async (e) => {
                                          ...
                                          <td className="text-center">
                                              <select className="form-select form-select-sm" value={w.status} onChange={e => updateWarrantyStatus(w.id, e.target.value)}>
                                                  <option value="CHECKING">Đang kiỒm tra</option>
                                                  <option value="PROCESSING">Đang sửa chữa/Đ�"i m�:i</option>
                                              </select>
                                          </td>
                                          <td className="text-end px-4">
                                  ...
                                  {warranties.filter(w => w.status === "CHECKING" || w.status === "PROCESSING").length === 0 && <tr><td colSpan={5} className="text-center p-4 text-muted small">Hi�!n không có máy nào �ang sửa chữa.</td></tr>}
                              </tbody>
                          </table>
                      </div>
                  </div>
                              ...
                              <tbody>
                                  {getFilteredWarranties(warranties.filter(w => w.status === "DONE")).map(w => (
                                      <tr key={w.id}>
                                          ...
                                          <td>{w.user?.name}</td>
                                          <td className="small italic text-success">{w.resolution || "Đã xử lý t�t"}</td>
                                          <td className="text-end px-4">
                                              <button className="btn btn-sm btn-outline-danger" onClick={() => deleteWarranty(w.id)}><i className="bi bi-trash"></i> Xóa l�9ch sử</button>
                                          </td>
                                      </tr>
                                  ))}
          ...
          {activeTab === "CONTACTS" && (
              <div>
                  <div className="d-flex justify-content-between align-items-center mb-4">
                      <h3 className="fw-bold mb-0"><i className="bi bi-chat-left-dots text-primary me-2"></i>H�"p Thư Liên H�! Tư Vấn</h3>
                      <div className="d-flex gap-2">
                          <span className="small text-muted align-self-center me-2">Sắp xếp theo:</span>
                          <button className={`btn btn-sm ${contactSort.key === 'createdAt' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => handleSortContact('createdAt')}>M�:i nhất</button>
          ...
          {selectedSale && (
            <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ background: "rgba(0,0,0,0.55)", zIndex: 2000 }} onClick={() => setSelectedSale(null)}>
              <div className="bg-white rounded-4 shadow-lg p-4" style={{ width: "min(900px, calc(100vw - 2rem))", maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div>
                    <div className="text-muted small">Hóa �ơn bán</div>
                ...
                <div className="row g-3 mb-4">
                  ...
                  <div className="col-md-4">
                    <div className="border rounded-3 p-3 h-100 bg-info bg-opacity-10 border-info border-opacity-25">
                      <div className="fw-bold mb-2 text-info"><i className="bi bi-geo-alt-fill me-1"></i>Đ�9a ch�0 giao hàng</div>
                      <div className="small fw-bold">{selectedSale.shippingAddress || "Chưa cập nhật ��9a ch�0"}</div>
                    </div>
                  </div>
                  <div className="col-md-4">
                    <div className="border rounded-3 p-3 h-100 bg-light">
                      <div className="fw-bold mb-2">T�"ng tiền</div>
                      <div className="fs-4 fw-bold text-primary">{selectedSale.total.toLocaleString()} �</div>
                  ...
                  <table className="table align-middle mb-0">
                    ...
                    <tbody>
                      {formatSaleItems(selectedSale).map((item) => {
                        ...
                        return (
                          <tr key={item.id}>
                            <td className="fw-semibold">{item.product?.name || "Sản phẩm �ã xóa"}</td>
                            ...
                            <td className="text-end fw-bold">{lineTotal.toLocaleString()} �</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <div className="mt-4 p-3 border rounded-3 bg-warning bg-opacity-10 border-warning border-opacity-25">
                  <label className="form-label fw-bold text-dark"><i className="bi bi-journal-text me-2"></i>Ghi chú n�"i b�" (Ch�0 nhân viên thấy)</label>
                  <textarea 
                    className="form-control border-warning border-opacity-50" 
                    rows={3} 
                    placeholder="Nhập ghi chú quan trọng về �ơn hàng này..."
            ...
            <footer className="mt-auto pt-5 pb-5 border-top border-light text-center flex-shrink-0">
               <p className="small text-muted mb-0 opacity-75 fw-bold">© 2026 Bản quyền thu�"c về H�! th�ng Quản tr�9 ABC XYZ</p>
               <div className="x-small text-muted mt-1 opacity-50"><i className="bi bi-shield-lock-fill me-1"></i>H�! th�ng bảo mật 256-bit chuẩn Doanh nghi�!p</div>
            </footer>
            {/* SAFE SCROLLING AREA */}
