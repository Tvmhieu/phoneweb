...
export async function GET() {
  try {
    const [saleOrders, pendingWarranties, pendingSales, totalProducts, totalCustomers] = await Promise.all([
    ...
    const revenueByMonth = new Map<string, { doanhThuBan: number }>();
    monthKeys.forEach((key) => revenueByMonth.set(key, { doanhThuBan: 0 }));
